...
- **MediaCube round 5 feedback addressed (CTF_01 + CTF_02).** Artifact: `cert/out/cut-the-fuse-playables.zip`, 9.85 MiB (10,328,295 bytes), sha256 `cea0131d994d484a…`. Verified against the staged zip: `verify-firstframe` 10/10, `verify-portal` (`PORTAL_ONLY=playables`) 15/15.
  - **CTF_01 — `firstFrameReady` test failed in the SDK Test Suite.** *"FirstFrameReady must be called either the game is rendering splash screen or loading screen, no need to finish loading the game."*
    - **Root cause**: the signal was produced by the game loop's first *painted* frame — it required `requestAnimationFrame` to fire and a non-zero canvas. The SDK test suite loads the game in an **off-screen iframe, where rAF never runs**, so the signal could never arrive while `gameReady` (which is not rAF-gated) still did. Reproduced against the pre-fix build: the observed order was literally **`gameReady → firstFrameReady`**, i.e. inverted — exactly "the firstFrameReady test fails".
    - **Fix**: added a real loading screen — `#loading-overlay` in `index.html`, styled in `style.css`, built from **static HTML+CSS** so it is painted on the browser's first frame with no JS, no font download and no image fetch on the critical path (the mark is a plain `<img>` that degrades to nothing; title, burn-bar and hint always show). `boot()` now calls `platform.signalFirstFrameReady()` as its **first statement**, with the splash already on screen — no `await`, no canvas, no rAF. The splash is dismissed (CSS fade, then removed, with a timer floor so a never-painted frame can't strand it) once the menu is interactable, alongside `gameReady`.
    - `Platform.signalGameReady()` now **retries `firstFrameReady` first**, so no call site can ever invert the order; and `signalFirstFrameReady()` retries rather than latching if the SDK namespace isn't up yet, keeping the `loadingFinished()` backstop useful.
    - Splash z-index is 150: above the modals (100) so it cross-fades over the menu card `openMenu()` has already put up, below `.host-shield` (200) so a host pause during loading still blocks input on top.
    - **Regression test** `tools/smoke/verify-firstframe.mjs` (10 checks): scenario A severs `requestAnimationFrame` before the page loads, reproducing the off-screen iframe, and asserts both signals still fire in order with the splash visibly up; scenario B asserts the splash comes down, the canvas is sized, and the dismissed splash no longer intercepts taps. **Fails on the pre-fix code** with `order=gameReady → firstFrameReady`.
  - **CTF_02 — fuse hiss continued after returning to the main menu.** *"The hissing sound of the fuse fails to stop and continues to play indefinitely after the player clicks the Menu button."*
    - **Root cause**: the `wick_crackle` loop is started/stopped from `GameLoop._update()`, and the in-game Menu button pauses via `game.setPaused(true)` → `gameState = PAUSED`. `_frame()` skips `_update()` while paused, so the `else stopLoop("wick_crackle")` branch never ran again and the loop was **orphaned** — nothing could ever stop it. A host/ad pause never reproduced this because that path additionally suspends the AudioContext (which silences it); only the Menu button did not.
    - **Fix**: `setPaused(true)` now stops gameplay loops (`stopAllLoops`) at the moment the game leaves the PLAYING state; `_update()` restarts the hiss on resume if anything is still burning. `resetLevel()` also clears loops so a retry or a level switch can never inherit or stack a second hiss.
    - **Regression test** `tools/smoke/verify-hiss-exit.mjs` (11 checks): starts a level with a live hiss loop, clicks the Menu button, and asserts the loop is gone (and stays gone 1.2s later, since a paused loop cannot resurrect itself), that PLAY resumes the same level and the hiss returns when a wick is burning, and that a second exit leaves nothing behind. **Fails on the pre-fix code** with `loops=[wick_crackle]`.
- **Re-verified (2026-09-21)**: `smoke`, `verify-portal` (31/31), `verify-firstframe` (10/10), `verify-hiss-exit` (11/11), `verify-ui`, `verify-rotation`, `verify-coverage`, `verify-coldboot-size` all pass, on the source tree **and** on the staged zip. Evidence: `tools/smoke/verify-splash-portrait.png`, `verify-splash-landscape.png`.
